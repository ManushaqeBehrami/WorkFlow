    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-6 md:grid-cols-[1.2fr_1fr]">
        ...
        <div className="rounded-2xl border border-slate-200/70 bg-white/80 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/70">
          ...
          {latestDoc?.id ? (
            <Link
              className="mt-4 block w-full rounded-lg bg-slate-900 px-4 py-2 text-center text-sm font-semibold text-white hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-slate-200"
              to={`/documents/${latestDoc.id}`}
            >
              View contract
            </Link>
          ) : (
            <div className="mt-4 rounded-lg border border-dashed border-slate-300 px-4 py-2 text-center text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
